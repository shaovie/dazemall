...
<body class="no-skin">
	...
	<form id="save-form" action="<?php echo $action?>" method="post" enctype="multipart/form-data" class="form-horizontal">	
		<input type="hidden" id="valueId" name="valueId" value="<?php echo $valueId?>">
		<input type="hidden" id="attrId" name="attrId" value="<?php echo $attrId?>">
				
		<div class="form-group">
			<label class="col-sm-2 control-label no-padding-left"> SKU值名称</label>
			<div class="col-sm-9">												
				<input type="text" id="skuValue" name="skuValue" class="col-xs-10 col-sm-2" value="<?php if (!empty($info['value'])){echo $info['value'];}?>">
			</div>
		</div>
